...
def main():
    ...
    if os.environ.get('GITHUB_ACTIONS')=='true':
        ...
        if 'files_changed' in locals() and files_changed is not None:
            forecasts_changed.extend([f"./{file.filename}" for file in files_changed if file.filename.startswith('data-processed') and (file.filename.endswith('.csv') or file.filename.endswith('.txt'))])
