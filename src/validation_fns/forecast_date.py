...
def filename_match_forecast_date(filepath):
  ...
  forecast_date_columns = set(list(df['forecast_date']))
  if len(forecast_date_columns) > 1:
    return (
      True,
      (
        f"FORECAST DATE ERROR: {filepath} has multiple forecast dates: "
        f"{forecast_date_columns}. Forecast date must be unique."
      )
    )
  else:
    forecast_date_column = forecast_date_columns.pop()
